...
raw_data <- read_csv("C:/Users/zzpen/Documents/OneDrive - University of Toronto/UofT/Study Material/2021/Fall/STA304/Final Project/Input/data/AAGkcsi6.csv")
dict <- read_lines("C:/Users/zzpen/Documents/OneDrive - University of Toronto/UofT/Study Material/2021/Fall/STA304/Final Project/Input/data/gss_dict.txt", skip = 18) # skip is because of preamble content
# Now we need the labels because these are the actual responses that we need
labels_raw <- read_file("C:/Users/zzpen/Documents/OneDrive - University of Toronto/UofT/Study Material/2021/Fall/STA304/Final Project/Input/data/gss_labels.txt")
...
# Match the variables from survey data
gss <- gss %>%
  select(age,
         total_children,
         sex,
...
# Remove all NA and Don't know observations
gss <- na.omit(gss)
gss <- subset(gss, education!="NA")
gss <- subset(gss, place_birth_canada!="NA")
gss <- subset(gss, place_birth_canada!="Don't know")

# Hot-code variables
gss$bachelor <- NA
gss$bornin_canada <- NA
gss$male <- NA
gss$have_children <- NA
gss<-
  gss %>%
  mutate(bachelor = 
           ifelse(education == "Bachelor's degree (e.g. B.A., B.Sc., LL.B.)" | education == "University certificate, diploma or degree above the bach...", 1, 0),
         bornin_canada = 
           ifelse(place_birth_canada == "Born in Canada", 1, 0),
         male = 
           ifelse(sex == "Male", 1, 0),
         have_children = 
           ifelse(total_children == 0, 0, 1)
  )
gss <- subset(gss, select = -c(education, place_birth_canada, marital_status, sex, total_children))

# Create age group
# Keep only eligible voters
gss$age <- as.integer(floor(gss$age))
gss <- subset(gss, age>17)
gss$age_under_30 <- NA
gss<- mutate(gss, age_under_30 = 
           ifelse(age <= 30, 1, 0))
gss <- subset(gss, select = -c(age))

census_data <- 
  gss %>%
  count(province, bachelor, bornin_canada, have_children, male, age_under_30) %>%
  group_by(province, age_under_30, male) %>%
  filter(n >= 10)
